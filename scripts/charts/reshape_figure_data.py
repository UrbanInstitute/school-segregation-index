...
with open("%s/%s/data/cleaned_source.csv"%(env["PROJECT_PATH"], argv[1])) as sourceFile:
	...
	for row in reader:
		...
		rows.append([schoolId,level,row[h["gleaid"]],row[h["REV_Subreg"]], row[h["school_name"]],row[h["gleaname"]],row[h["maname"]],row[h["REV_City"]] ,state,row[h["population_school"]],row[h["minority_school"]],minorityPercent,neighbor_minority_percent, row[h["SCI_sys"]],schoolTypeString,compareMedian, row[h["lon"]], row[h["lat"]], row[h["radius_nbrsch"]], row[h["count_nbrsch"]] ])
